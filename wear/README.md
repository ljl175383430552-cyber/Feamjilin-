...
> **现场最快路径（推荐先看）**：多数手表**没有摄像头**，真正快的不是「手表扫码」，而是：
>
> 1. 手机打开 [`/join` 接入页](/join)（或电脑跑 `npm run field` 出局域网二维码）
> 2. 把 **网页版 `/w`** 链接发到手表浏览器 → **零安装立刻能拍**
> 3. 浏览器打不开的机型，再扫码下 APK / ADB 侧载
>
> 详细说明见下方「路径 0」与仓库根目录的 `npm run field`。

手表不像手机有应用商店那么方便，按设备选一条即可。

### 路径 0 · 扫码 / 短链 · 网页零安装（换表最快）

适合：临时换表、新品牌、剧组来不及装 App、只要当天能拍。

| 步骤 | 做什么 |
|------|--------|
| 1 | 手机打开工具 → 设置里点手表图标，或直接访问 `/join` |
| 2 | 「网页即用」页亮出二维码；用**另一台手机**扫，或复制链接到手表浏览器 |
| 3 | 手表打开 `/w` → 选色 / 调点 → **锁定屏幕** → 开拍 |
| 4 | 剧组没外网时：电脑执行 `npm run field`，终端打印局域网二维码 |

网页版与原生版操作一致（锁定 + 长按 3 秒解锁）。原生包提供更稳的常亮、实体键与离线驻留。
